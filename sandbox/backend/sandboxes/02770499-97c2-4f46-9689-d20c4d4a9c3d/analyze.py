...
for col in result["numeric_columns"]:
    try:
        ...
        # 确保文字框可见
        text_box = dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.9, edgecolor='black', linewidth=1)
        ax1.text(0.5, 0.95, '【直方图】', transform=ax1.transAxes, 
                 fontsize=16, ha='center', bbox=text_box)
        
        # 添加统计数据文本框
        stats_box = dict(boxstyle='round,pad=0.3', facecolor='lightyellow', alpha=0.9, edgecolor='orange', linewidth=1)
        stat_text = f"平均值: {df[col].mean():.2f}
中位数: {df[col].median():.2f}
标准差: {df[col].std():.2f}"
        ax1.text(0.5, 0.15, stat_text, transform=ax1.transAxes, 
                 fontsize=12, ha='center', va='center', bbox=stats_box)
        ...
        # 确保文字框可见
        ax2.text(0.5, 0.95, '【箱线图】', transform=ax2.transAxes, 
                 fontsize=16, ha='center', bbox=text_box)
        
        # 添加箱线图统计数据
        box_stats = f"Q1: {df[col].quantile(0.25):.2f}
Q3: {df[col].quantile(0.75):.2f}
异常值: {len(df[(df[col] < df[col].quantile(0.25) - 1.5*(df[col].quantile(0.75)-df[col].quantile(0.25))) | (df[col] > df[col].quantile(0.75) + 1.5*(df[col].quantile(0.75)-df[col].quantile(0.25)))])}"
        ax2.text(0.5, 0.15, box_stats, transform=ax2.transAxes, 
                 fontsize=12, ha='center', va='center', bbox=stats_box)
