...
# 数据预处理，确保销售额列的数据类型为数值型
df['销售额'] = df['销售额'].astype(float)

# 找出销售额最高的前三个地区
top_three_regions = df.nlargest(3, '销售额')[['地区', '销售额']]
result_data = top_three_regions.to_dict(orient='records')

# 可视化
plt.figure(figsize=(12, 8))
sns.barplot(x='地区', y='销售额', data=top_three_regions, palette='pastel')
...
# 保存结果到JSON文件
result = {
    'top_three_regions': result_data,
    'charts': {'top_three_regions_sales': 'data/charts/top_three_regions_sales.png'}
}
